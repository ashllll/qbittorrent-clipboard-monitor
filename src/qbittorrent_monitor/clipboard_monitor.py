...
class ClipboardMonitor:
    ...
    def __init__(self, qbt: QBittorrentClient, config: AppConfig):
        ...
        self.magnet_pattern = re.compile(
            r"^magnet:\?xt=urn:btih:[0-9a-fA-F]{40,}.*", re.IGNORECASE
        )

        # XXXClub URL正则模式
        self.xxxclub_pattern = re.compile(
            r"https?://(?:www\.)?xxxclub\.to/torrents/search/.*", re.IGNORECASE
        )
    ...
    async def _monitor_cycle(self):
        ...
        try:
            ...
            # 检查是否为新内容且为磁力链接
            if (
                current_clip != self.last_clip
                and current_clip
                and self.magnet_pattern.match(current_clip.strip())
            ):
            ...
            # 检查是否为网页URL(XXXClub或通用URL)
                ...
                and (
                    ...
                    or self.url_pattern.match(current_clip.strip())
                )
            ):
            ...
            if self.consecutive_errors <= 3:
                self.logger.warning(
                    f"监控循环错误 ({self.consecutive_errors}/3): {str(e)}"
                )
            else:
    ...
    async def _process_magnet(self, magnet_link: str):
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(
                    f"✅ 成功添加种子: {record.torrent_name} -> {category}"
                )
            else:
        ...
        try:
            if await self.qbt._is_duplicate(record.torrent_hash):
                ...
                await self.notification_manager.send_duplicate_notification(
                    record.torrent_name, record.torrent_hash
                )
    ...
    async def _classify_torrent(self, record: TorrentRecord) -> str:
        ...
        try:
            category = await self.ai_classifier.classify(
                record.torrent_name, self.config.categories
            )
            ...
            self.logger.info(
                f"🧠 分类结果: {record.torrent_name[:50]}... -> {category} ({record.classification_method})"
            )
            return category
    ...
    async def _add_torrent_to_client(self, record: TorrentRecord) -> bool:
        ...
        try:
            # 准备要传递给客户端的额外参数
            torrent_params = {"paused": self.config.add_torrents_paused}
            ...
            success = await self.qbt.add_torrent(
                record.magnet_link, record.category or "other", **torrent_params
            )
        ...
        if self.config.notifications.console.enabled:
            ...
            if self.config.notifications.console.show_statistics:
                if self.notification_manager.use_colors:
                    ...
                    print(
                        f"{Fore.CYAN}总处理数: {Fore.WHITE}{self.stats.get('total_processed', 0)}"
                    )
                    print(
                        f"{Fore.GREEN}成功添加: {Fore.WHITE}{self.stats.get('successful_adds', 0)}"
                    )
                    print(
                        f"{Fore.RED}添加失败: {Fore.WHITE}{self.stats.get('failed_adds', 0)}"
                    )
                    print(
                        f"{Fore.YELLOW}重复跳过: {Fore.WHITE}{self.stats.get('duplicates_skipped', 0)}"
                    )
                    print(
                        f"{Fore.MAGENTA}URL爬取: {Fore.WHITE}{self.stats.get('url_crawls', 0)}"
                    )
                    print(
                        f"{Fore.MAGENTA}批量添加: {Fore.WHITE}{self.stats.get('batch_adds', 0)}"
                    )
            ...
            farewell_lines = ["👋 qBittorrent剪贴板监控已停止", "感谢使用，再见!"]
    ...
    async def _handle_monitor_error(self, error: Exception):
        ...
        await self.notification_manager.send_torrent_failure(
            "系统错误", error_message, "system_error", ""
        )
            ...
            if result["success"]:
                ...
                if self.config.notifications.console.enabled:
                    if self.notification_manager.use_colors:
                        ...
                        print(
                            f"   找到种子: {Fore.WHITE}{stats.get('torrents_found', 0)}"
                        )
                        print(
                            f"   成功添加: {Fore.GREEN}{stats.get('torrents_added', 0)}"
                        )
                        print(
                            f"   重复跳过: {Fore.YELLOW}{stats.get('duplicates_skipped', 0)}"
                        )
