...
from .config import CategoryConfig, DeepSeekConfig
...
class BaseAIClassifier(ABC):
    ...
    @abstractmethod
    async def classify(
        self, torrent_name: Optional[str], categories: Dict[str, CategoryConfig]
    ) -> str:
        """分类种子名称"""
        pass
...
class DeepSeekClassifier(BaseAIClassifier):
    ...
    @retry(
        ...
        before_sleep=before_sleep_log(
            logging.getLogger("AIClassifier.Retry"), logging.INFO
        ),
    )
    async def _classify_with_retry(
        self, torrent_name: str, categories: Dict[str, CategoryConfig]
    ) -> str:
        """带重试机制的分类方法"""
        try:
            ...
            if completion.choices and completion.choices[0].message:
                ...
                if raw_category:
                    ...
                    self.logger.info(
                        f"AI分类结果: {torrent_name[:50]}... -> {category}"
                    )
                    return category
    ...
    def _build_prompt(
        ...
        if self.config.few_shot_examples:
            ...
            for example in self.config.few_shot_examples:
                examples.append(
                    f"示例: '{example['torrent_name']}' -> {example['category']}"
                )
    ...
    def _rule_based_classify(
        ...
        patterns = {
            "tv_episode": re.compile(
                r"s\d+e\d+|season\s+\d+|episode\s+\d+|\d+x\d+", re.IGNORECASE
            ),
            "movie_year": re.compile(
                r"\.(19|20)\d{2}\.|\((19|20)\d{2}\)|\[(19|20)\d{2}\]"
            ),
            "movie_quality": re.compile(
                r"1080p|720p|2160p|4k|uhd|bluray|web-?dl|hdtv", re.IGNORECASE
            ),
            "anime_fansub": re.compile(
                r"\[.*\].*\d+.*\[.*\]", re.IGNORECASE
            ),  # [字幕组]标题[质量]格式
        }
        ...
        if patterns["movie_year"].search(name_lower) or patterns[
            "movie_quality"
        ].search(name_lower):
        ...
        for cat_name, cat_config in categories.items():
                ...
                for rule in cat_config.rules:
                    ...
                    if rule_score > 0:
                        ...
                        matched_items.append(
                            f"{rule.get('type', 'unknown')}(+{rule_score})"
                        )
            ...
            if score > 0:
                ...
                category_scores[cat_name] = (
                    category_scores.get(cat_name, 0) + weighted_score
                )

                if matched_items:
                    self.logger.info(
                        f"{cat_name} 匹配: {', '.join(matched_items)}, 权重分数: {weighted_score:.1f}"
                    )
        ...
        if category_scores:
            ...
            scores_display = ", ".join(
                [f"{k}: {v:.1f}" for k, v in category_scores.items()]
            )
            self.logger.info(
                f"规则引擎分类结果: {best_category} (所有分数: {scores_display})"
            )
    ...
    def _apply_rule(
        self, rule: Dict[str, Any], original_name: str, lower_name: str
    ) -> int:
        """应用单个规则"""
