...
from .config import AppConfig, CategoryConfig, PathMappingRule, QBittorrentConfig
...
class QBittorrentClient:
    ...
    def __init__(
        self, config: QBittorrentConfig, app_config: Optional[AppConfig] = None
    ):
        self.config = config
    ...
    @retry(
        ...
        before_sleep=before_sleep_log(
            logging.getLogger("QBittorrent.Retry"), logging.INFO
        ),
    )
    async def login(self):
        ...
        try:
            self.logger.info(
                f"尝试登录qBittorrent: {self.config.host}:{self.config.port}"
            )
            async with self.session.post(url, data=data) as resp:
                if resp.status == 200:
                    ...
                    raise QBittorrentError(
                        f"登录失败: HTTP {resp.status} - {error_text}"
                    )
        ...
        try:
            async with self.session.get(url) as resp:
                if resp.status == 200:
                    ...
                    error_text = await resp.text()
                    raise QBittorrentError(
                        f"获取分类失败: HTTP {resp.status} - {error_text}"
                    )
            ...
            for name, config in categories.items():
                ...
                if name not in existing_categories:
                    ...
                    if existing_path != mapped_path:
                        self.logger.info(
                            f"更新分类路径: {name} (当前路径: {existing_path} -> 新路径: {mapped_path})"
                        )
                        await self._update_category(name, mapped_path)
                    else:
                        self.logger.info(
                            f"分类路径未变，跳过更新: {name} (路径: {existing_path})"
                        )
            ...
            for mapping in self.config.path_mapping:
                if original_path.startswith(mapping.source_prefix):
                    mapped_path = original_path.replace(
                        mapping.source_prefix, mapping.target_prefix, 1
                    )
                    self.logger.debug(
    ...
    @retry(
        ...
        retry=retry_if_exception_type((NetworkError, QbtRateLimitError)),
        before_sleep=before_sleep_log(
            logging.getLogger("QBittorrent.AddTorrent"), logging.INFO
        ),
    )
            ...
            async with self.session.post(url, data=data) as resp:
                if resp.status == 200:
                    ...
                    if response_text != "Fails.":
                        # 尝试获取文件名但不阻塞主流程
                        try:
                            torrent_info = await self.get_torrent_properties(
                                torrent_hash
                            )
                    ...
                    error_text = await resp.text()
                    raise QBittorrentError(
                        f"添加种子失败: HTTP {resp.status} - {error_text}"
                    )
        ...
        try:
            ...
            async with self.session.post(url, data=data) as resp:
                ...
                if resp.status == 200:
                    ...
                    self.logger.warning(
                        f"⚠️ 种子重命名失败 (HTTP {resp.status}): {response_text}"
                    )

                    # 尝试备用方法：使用setName端点
                    return await self._set_torrent_name_alternative(
    ...
    async def _set_torrent_name_alternative(
        self, torrent_hash: str, new_name: str
    ) -> bool:
        """备用重命名方法：使用setName端点"""
        try:
            ...
            async with self.session.post(url, data=data) as resp:
                ...
                if resp.status == 200:
                    ...
                    self.logger.warning(
                        f"⚠️ 备用重命名也失败 (HTTP {resp.status}): {response_text}"
                    )
                    return False
        except Exception as e:
