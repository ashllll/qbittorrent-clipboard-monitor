...
class QBittorrentDownloadHelper:
    ...
    async def _initialize(self):
        ...
        try:
            self.qbt_client = QBittorrentClient(
                self.config.qbittorrent, app_config=self.config
            )
        ...
        print_step(
            5,
            total_steps,
            f"正在连接qBittorrent ({self.config.qbittorrent.host}:{self.config.qbittorrent.port})...",
        )
        try:
    ...
    def _show_startup_info(self):
        ...
        print_separator()
        print_with_flush(
            f"📡 qBittorrent: {self.config.qbittorrent.host}:{self.config.qbittorrent.port}"
        )
        print_with_flush(f"👤 用户: {self.config.qbittorrent.username}")
        print_with_flush(
            f"🧠 AI分类: {'✅ 已启用 (DeepSeek)' if self.config.deepseek.api_key else '❌ 未配置'}"
        )
