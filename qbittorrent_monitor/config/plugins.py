...
class PluginConfig:
    ...
    def validate(self) -> None:
        ...
        if not isinstance(self.enabled, bool):
            raise ConfigurationError(f"PLUGINS_ENABLED 必须是布尔值，当前值: {self.enabled}")
        
        if not isinstance(self.auto_enable, bool):
            raise ConfigurationError(f"PLUGINS_AUTO_ENABLE 必须是布尔值，当前值: {self.auto_enable}")
        
        if not isinstance(self.auto_discover, bool):
            raise ConfigurationError(f"PLUGINS_AUTO_DISCOVER 必须是布尔值，当前值: {self.auto_discover}")
        
        if self.plugins_dir and not isinstance(self.plugins_dir, str):
            raise ConfigurationError(f"PLUGINS_DIR 必须是字符串，当前值: {self.plugins_dir}")
        
        # 检查插件目录是否存在（如果配置）
        if self.plugins_dir:
            path = Path(self.plugins_dir)
            if path.exists() and not path.is_dir():
                raise ConfigurationError(f"PLUGINS_DIR 必须是目录: {self.plugins_dir}")
